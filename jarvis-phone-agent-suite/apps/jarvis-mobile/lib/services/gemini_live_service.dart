...
You are also connected to the JARVIS Phone Agent suite:
- Siri2: rooted on-device operator for commands, notification handling, scheduling, and lock control
- PhonePilot: ADB-based Android operator for app navigation and multi-step phone tasks

When a user asks you to actually operate the phone, call the suite tools instead of only describing what should happen.
- Use run_phone_agent_task with mode "phonepilot" for opening apps, navigating UI, tapping through flows, taking screenshots, and other ADB-safe device control.
- Use run_phone_agent_task with mode "siri2" for rooted-phone requests, notification triage, scheduler-oriented requests, and direct command execution on the phone itself.
...
When users ask to see notes, reminders, calendar events, or lists, use the appropriate tool calls to display them on screen. Be conversational but efficient, and always aim to be genuinely helpful while maintaining awareness of the user's safety as a Mars astronaut.'''
          ...
          'tools': [
            {
              'functionDeclarations': [
                ...
                },
                {
                  ...
                  'description': 'Execute a real phone automation task using the merged JARVIS suite. Use mode "siri2" for rooted on-device automation like notifications, scheduler, and command execution. Use mode "phonepilot" for ADB-driven app control on a connected Android device.',
                  'parameters': {
                    'type': 'object',
                    'properties': {
                      'mode': {
                        'type': 'string',
                        'enum': ['siri2', 'phonepilot'],
                ...
                },
                {
                  ...
                  'description': 'Check whether Siri2 and PhonePilot backends are currently reachable before attempting automation.',
                  'parameters': {
                    'type': 'object',
                    'properties': {}
                  }
                },
                {
                  ...
                  'description': 'Emergency stop for the rooted Siri2 backend. Use this if the user asks to stop or cancel an in-flight device action.',
                  'parameters': {
                    'type': 'object',
                    'properties': {}
                  }
  ...
  void _handleFunctionCall(Map<String, dynamic> functionCall) {
    ...
    if (functionId != null) {
      sendFunctionResponse(functionId, functionName, result);
    }
  }

  /// Send function response back to Gemini
  Future<void> sendFunctionResponse(String functionId, String functionName, String result) async {
