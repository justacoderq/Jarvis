...
import 'jigglewiggle_screen.dart';
...
class _JarvisHUDScreenState extends State<JarvisHUDScreen>
  ...
  BackendHealth _siri2Health = const BackendHealth(ok: false, label: 'SIRI2 OFFLINE', detail: 'Not checked');
  BackendHealth _phonePilotHealth = const BackendHealth(ok: false, label: 'PHONEPILOT OFFLINE', detail: 'Not checked');
  SuiteExecutionMode _selectedMode = SuiteExecutionMode.siri2;
  bool _isExecutingBackendCommand = false;
  bool _wakeWordAvailable = false;
  bool _resumeWakeWordWhenPlaybackStops = false;
  bool _siri2BackgroundAgentsInitialized = false;
  ...
  void _initializeServices() {
    ...
    _suiteBackendService = SuiteBackendService(
      siri2BaseUrl: widget.siri2BaseUrl,
      phonePilotBaseUrl: widget.phonePilotBaseUrl,
      jiggleWiggleBaseUrl: widget.jiggleWiggleBaseUrl,
    );
  ...
  Future<void> _refreshBackendHealth() async {
    final siri2 = await _suiteBackendService.checkSiri2Health();
    final phonePilot = await _suiteBackendService.checkPhonePilotHealth();
    if (!mounted) return;
    setState(() {
      _siri2Health = siri2;
      _phonePilotHealth = phonePilot;
    });

    if (siri2.ok && !_siri2BackgroundAgentsInitialized) {
      final initialized = await _suiteBackendService.ensureSiri2NotificationWatcher(
        defaultPackages: const ['*'],
      );
      if (!mounted) return;
        if (initialized) {
          setState(() {
            _siri2BackgroundAgentsInitialized = true;
            _siri2Health = const BackendHealth(
              ok: true,
              label: 'SIRI2 ONLINE',
  ...
  Future<void> _refreshProductivityInsights() async {
    try {
      final notificationStatus = await _suiteBackendService.getSiri2NotificationStatus();
      final notificationLog = await _suiteBackendService.getSiri2NotificationLog();
      final schedulerStatus = await _suiteBackendService.getSiri2SchedulerStatus();
      final schedulerLog = await _suiteBackendService.getSiri2SchedulerLog();
  ...
  Future<void> _suggestMicroBreak({String? prompt}) async {
    try {
      final breakPlan = await _suiteBackendService.generateJiggleMicroBreak(
        prompt: prompt ??
            (_focusModeEnabled
  ...
  Future<void> _toggleFocusMode() async {
    ...
    _addComponentWithAnimation(
      UIComponent.card(
        ...
        content: 'JARVIS will track this session and recommend a JiggleWiggle micro-break soon.',
      ),
    );
  }
  ...
  Future<void> _submitBackendCommand() async {
    ...
    final result = _selectedMode == SuiteExecutionMode.siri2
        ? await _suiteBackendService.executeSiri2Command(prompt)
        : await _suiteBackendService.executePhonePilotGoal(prompt);
    ...
    final component = UIComponent.card(
      ...
      subtitle: _selectedMode == SuiteExecutionMode.siri2 ? 'Rooted phone operator' : 'ADB autonomy engine',
  ...
  Future<void> _openJiggleWiggle() async {
    ...
    await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => JiggleWiggleScreen(baseUrl: widget.jiggleWiggleBaseUrl),
      ),
    );
  ...
  Future<void> _emergencyStop() async {
    final released = await _suiteBackendService.releaseSiri2Lock();
    if (!mounted) return;
    final component = UIComponent.card(
      id: 'emergency_stop_${DateTime.now().millisecondsSinceEpoch}',
      title: released ? 'EMERGENCY STOP SENT' : 'EMERGENCY STOP FAILED',
      subtitle: 'Siri2 lock control',
      content: released ? 'Device lock released successfully.' : 'Could not reach Siri2 lock endpoint.',
    );
    _addComponentWithAnimation(component);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          statusRow('SIRI2', _siri2Health, Colors.tealAccent),
          statusRow('PHONEPILOT', _phonePilotHealth, Colors.orangeAccent),
          TextButton(
      ...
      child: Column(
        ...
        children: [
          ...
          SegmentedButton<SuiteExecutionMode>(
            segments: const [
              ButtonSegment(
                value: SuiteExecutionMode.siri2,
                label: Text('Siri2'),
                icon: Icon(Icons.phone_android),
              ),
              ButtonSegment(
                value: SuiteExecutionMode.phonePilot,
                label: Text('PhonePilot'),
          ...
          TextField(
            ...
            decoration: InputDecoration(
              hintText: _selectedMode == SuiteExecutionMode.siri2
  ...
  Future<void> _handleSuiteToolCall(Map<String, dynamic> toolCall) async {
    ...
    switch (function) {
      case 'run_phone_agent_task':
        final mode = (toolCall['mode'] as String? ?? 'siri2').toLowerCase();
        ...
        final component = UIComponent.card(
          ...
          title: '${result.source} ${result.success ? 'COMPLETE' : 'ERROR'}',
          subtitle: mode == 'phonepilot' ? 'ADB autonomy engine' : 'Rooted phone operator',
          content: result.message,
        );
        _addComponentWithAnimation(component);
        await _refreshBackendHealth();
      ...
      case 'stop_phone_agent':
        ...
        await _geminiService.sendFunctionResponse(
          functionId,
          function,
          released ? 'Siri2 emergency stop sent successfully.' : 'Failed to reach Siri2 emergency stop endpoint.',
        );
        return;
    }
  }
