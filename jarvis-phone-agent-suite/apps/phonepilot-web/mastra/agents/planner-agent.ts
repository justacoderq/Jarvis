  ...
  try {
    ...
    if (!parsed) {
      ...
      if (text_lower.includes('done') || text_lower.includes('complete')) {
        return createReliableFallbackPlan(goal, currentState, lastAction, recentActions);
      }
    ...
    return createReliableFallbackPlan(goal, currentState, lastAction);
  }
}
...
function createReliableFallbackPlan(goal: string, currentState: any, lastAction?: any, recentActions?: any[]) {
  console.log("Using reliable fallback planning - requiring visible evidence");
  ...
  const recentFailures = actions.slice(-3).filter(a => !a.success).length;
  const goalLower = goal.toLowerCase();
  const visibleText = JSON.stringify(currentState?.relevantNodes || currentState?.uiNodes || []).toLowerCase();

  if ((goalLower.includes('wi-fi') || goalLower.includes('wifi')) &&
      (visibleText.includes('wi-fi') || visibleText.includes('wifi'))) {
    return {
      status: "done",
      summary: "Wi-Fi appears on screen, so the navigation goal looks complete."
    };
  }
  ...
  if (recentFailures >= 2) {
    return {
      ...
      nextStep: {
        action: "screenshot",
        params: { path: "/sdcard/reassess.png" },
        rationale: "Several actions failed, so gather current visual evidence before continuing"
      }
    };
  }

  if (stepNumber <= 2 || successfulActions > 0) {
    return {
      status: "continue",
      nextStep: {
        action: "screenshot",
        params: { path: "/sdcard/verification_state.png" },
        rationale: "Need visible evidence before declaring the goal complete"
      }
    };
  }
  ...
  return {
    status: "blocked",
    error: "Could not verify goal completion from the current tablet UI state.",
    suggestedFix: "Take a screenshot, inspect the current screen, and choose the next selector-based action."
  };
}
