    ...
    try {
      ...
      if (this.shouldTakeScreenshot(context)) {
        ...
        const screenshotResult = await ADB.mediaScreenshot.execute({
          context: { 
            path: `/sdcard/phonepilot_step_${context.currentStep}.png`,
...
`;
  ...
  private heuristicGoalCompletion(context: RunContext): { isComplete: boolean; summary?: string } {
    ...
    // Check for common goal patterns and their completion signals
    const goalPatterns = [
      {
        pattern: /like.*profile|swipe.*right/i,
        completionActions: ['tapByDesc', 'tapByText'],
        completionParams: ['like', 'heart', 'yes', 'send'],
        minActions: 2, // Require at least 2 actions: initial like + confirmation
        requiresConfirmation: true
      },
      {
        pattern: /open.*app|launch.*app/i,
        completionActions: ['launchByAppName', 'launch', 'monkeyLaunch'],
        completionParams: [],
        minActions: 1
      },
      {
        pattern: /take.*photo|capture.*image|take.*picture/i,
        completionActions: ['tapByDesc', 'tapByText'],
        completionParams: ['capture', 'shutter', 'take', 'picture'],
        minActions: 1  // Just need 1 successful picture action
      },
      {
        pattern: /open.*camera.*flip.*take|camera.*front.*picture/i,
        completionActions: ['tapByDesc', 'tapByText'],
        completionParams: ['take', 'picture', 'capture', 'shutter'],
        minActions: 1  // After flipping, just need 1 picture
      },
      {
        pattern: /send.*message|share/i,
        completionActions: ['tapByDesc', 'tapByText', 'shareImage'],
        completionParams: ['send', 'share'],
        minActions: 2
      }
    ];
    
    // Check if goal matches any pattern and has appropriate completion actions
    for (const pattern of goalPatterns) {
      if (pattern.pattern.test(context.goal)) {
        const relevantActions = successfulActions.filter(action => 
          pattern.completionActions.includes(action.action) &&
          (pattern.completionParams.length === 0 || 
           pattern.completionParams.some(param => 
             JSON.stringify(action.params).toLowerCase().includes(param.toLowerCase())
           ))
        );
        
        if (relevantActions.length >= pattern.minActions) {
          return {
            isComplete: true,
            summary: `Goal pattern "${pattern.pattern.source}" completed with ${relevantActions.length} relevant actions`
          };
        }
      }
    }
    ...
    const isMultiStepGoal = goalLower.includes(' and ') || 
                           goalLower.includes(',') ||
                           goalLower.split(' ').length > 4;
    
    if (isMultiStepGoal && successfulActions.length >= 3) {
      return {
        isComplete: true,
        summary: `Multi-step goal likely complete after ${successfulActions.length} successful actions`
      };
    }
    
    if (!isMultiStepGoal && successfulActions.length >= 2) {
      return {
        isComplete: true,
        summary: `Simple goal likely complete after ${successfulActions.length} successful actions`
      };
    }
