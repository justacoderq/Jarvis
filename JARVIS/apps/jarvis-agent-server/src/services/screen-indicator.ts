...
const NOTIFICATION_ID = "inboxops_agent_active";
const NOTIFICATION_TAG = "inboxops_lock";
...
class ScreenIndicator {
  ...
  private async showNotification(): Promise<void> {
    ...
    await executeShell(
      `cmd notification post -t "InboxOps Agent Active" ` +
