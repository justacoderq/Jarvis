...
const jarvisAgentProvider = (process.env.JARVIS_AGENT_PROVIDER || process.env.SIRI2_PROVIDER || "openai").toLowerCase();
...
if (!providerKeys[jarvisAgentProvider]) {
  console.error(
    "\x1b[31mNo credentials found.\x1b[0m\n\n" +
    `  Selected provider: ${jarvisAgentProvider}\n` +
    "  Set the matching API key in your environment or .env file.\n" +
    "  Supported keys: OPENAI_API_KEY, XAI_API_KEY, GROQ_API_KEY, CEREBRAS_API_KEY\n"
  );
  process.exit(1);
}
