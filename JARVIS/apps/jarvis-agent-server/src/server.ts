...
app.get("/health", (_req, res) => {
  res.json({ status: "ok", agent: "inboxops", uptime: process.uptime() });
});
...
app.post("/notifications/test", (req, res) => {
  const title = typeof req.body.title === "string" ? req.body.title : "Demo notification";
  const text = typeof req.body.text === "string" ? req.body.text : "Open the relevant app and handle this request safely.";
  const packageName = typeof req.body.packageName === "string" ? req.body.packageName : "com.discord";
  const actions = Array.isArray(req.body.actions)
    ? req.body.actions.map((value: unknown) => String(value))
    : [];

  ctx.notificationQueue.enqueue({
    key: `demo-${Date.now()}`,
    packageName,
    title,
    text,
...
app.listen(PORT, "0.0.0.0", () => {
  console.log(`InboxOps HTTP server listening on 0.0.0.0:${PORT}`);
});
