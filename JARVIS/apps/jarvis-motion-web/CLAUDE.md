...
```
YouTube URL → /api/download (SSE: progress → done → classified) → /tmp/jarvis-motion/{id}.mp4
...
- **`app/api/download/route.ts`** — POST endpoint. Spawns yt-dlp, parses stdout for progress, streams SSE events (`progress`, `done`, `classified`, `error`). Caches to `/tmp/jarvis-motion/`.
...
- **Video ID validation:** Regex `^[a-zA-Z0-9_-]{11}$` in download route. Files stored at `/tmp/jarvis-motion/{videoId}.mp4`.
