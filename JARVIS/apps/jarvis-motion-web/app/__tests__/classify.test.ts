 ...
 *
 * The classifyVideo function calls OpenAI to determine
...
describe("classifyVideo", () => {
  ...
  describe("unit tests with mocked metadata", () => {
    ...
    it("defaults to 'dance' on ambiguous response", async () => {
      mockModelResponse("something unexpected");
      const result = await classifyVideo(
        "Random Video Title",
        "Some description"
      );
  ...
  describe("fallback behavior", () => {
    it("returns 'dance' when OPENAI_API_KEY is missing", async () => {
  ...
  describe("integration tests (require yt-dlp + OPENAI_API_KEY)", () => {
    // These tests call real external services — skip in CI
    const hasOpenAIKey = !!process.env.OPENAI_API_KEY_REAL;
    const itIntegration = hasOpenAIKey ? it : it.skip;
    ...
    itIntegration("classifies dance short (mWQGrkm86Ps) as dance", async () => {
      ...
      vi.stubEnv("OPENAI_API_KEY", process.env.OPENAI_API_KEY_REAL!);
