...
- `InboxOps` for rooted Android command execution, notification triage, and scheduling
- `TaskRunner` for ADB-based multi-step device control
- `MoveBreak` for movement, posture, and micro-break guidance
...
- a suggested MoveBreak session

### 2. Notification Inbox and Triage Surface

Expose InboxOps notification intelligence directly in the mobile app:
...
### 4. MoveBreak Micro-Break Integration

Treat MoveBreak as a productivity feature instead of a separate fitness demo:

- generate a short desk/stretch/mobility recommendation
- suggest routines based on focus fatigue
- provide a one-tap entry into MoveBreak from JARVIS
...
- a simple MoveBreak productivity API for micro-break generation
