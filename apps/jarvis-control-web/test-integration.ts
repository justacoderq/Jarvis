#!/usr/bin/env tsx
/**
 * Integration test script for TaskRunner
 * Tests the core functionality without requiring an actual Android device
 */

import { mastra } from "./mastra";
import { executeSingleAction } from "./mastra/agents/control-agent";
...
async function testMastraSetup() {
  ...
  try {
    ...
    if (!phonePilotAgent) throw new Error("TaskRunner agent not registered");
...
async function runIntegrationTests() {
  console.log("Starting TaskRunner integration tests");
  ...
  if (passed === total) {
    console.log("All tests passed. TaskRunner is ready to use.");
  } else {
  ...
  console.log("6. Visit http://localhost:3000/control");
