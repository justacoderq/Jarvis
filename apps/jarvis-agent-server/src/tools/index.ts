...
function parseScreenState(powerOutput: string, displayOutput: string) {
  const combined = `${powerOutput}\n${displayOutput}`.toLowerCase();
  const screenOn =
    combined.includes("state=on") ||
    combined.includes("screen_state_on") ||
    combined.includes("interactive=true") ||
    combined.includes("mwakefulness=awake");
  const screenOff =
    combined.includes("state=off") ||
    combined.includes("interactive=false") ||
    combined.includes("mwakefulness=asleep");

  if (screenOn) return { screenOn: true, screenState: "on" };
  if (screenOff) return { screenOn: false, screenState: "off" };
  return { screenOn: null, screenState: "unknown" };
}

function parseKeyguardState(policyOutput: string, focusedWindowOutput: string, screenOn: boolean | null) {
  const combined = `${policyOutput}\n${focusedWindowOutput}`.toLowerCase();
  const keyguardShowing =
    combined.includes("showing=true") ||
    combined.includes("mshowinglockscreen=true") ||
    combined.includes("keyguard");
  const keyguardHidden =
    combined.includes("showing=false") ||
    combined.includes("mshowinglockscreen=false") ||
    combined.includes("keyguardshowing=false");

  let lockState = "unknown";
  if (screenOn === false) {
    lockState = "locked";
  } else if (keyguardShowing) {
    lockState = "lockscreen_visible";
  } else if (keyguardHidden) {
    lockState = "unlocked";
  }

  return {
    keyguardShowing: keyguardShowing ? true : keyguardHidden ? false : null,
    lockState,
  };
}
...
export const toolDefs = [
  ...
  defineTool(
    "run_shell",
    "Execute a shell command, preferring root via su -c when available and falling back to a normal shell otherwise.",
    {
      type: "object",
      properties: {
  ...
  defineTool(
    ...
    { type: "object", properties: {} },
    async () => {
      const r = await executeShell("screencap -p /sdcard/inboxops_screenshot.png", { timeout: 10_000 });
      if (r.exitCode !== 0) return JSON.stringify({ ok: false, error: r.stderr });
      const b64 = await executeShell("cat /sdcard/inboxops_screenshot.png | base64", {
        timeout: 15_000,
  ...
  defineTool(
    "get_device_info",
    "Get explicit device status: battery, WiFi, whether the screen is on, whether the lock screen appears visible, and the current foreground app.",
    { type: "object", properties: {} },
    async () => {
      const [battery, wifi, power, display, windowPolicy, focusedWindow, activity] = await Promise.all([
        executeShell("dumpsys battery | grep -E 'level|status|plugged'"),
        executeShell("dumpsys wifi | grep 'Wi-Fi is' | head -1"),
        executeShell("dumpsys power | grep -E 'Display Power: state=|mWakefulness=|mInteractive=' | head -5"),
        executeShell("dumpsys display | grep 'mScreenState' | head -1"),
        executeShell("dumpsys window policy | grep -E 'showing=|mShowingLockscreen|keyguard' | head -20"),
        executeShell("dumpsys window windows | grep -E 'mCurrentFocus|mFocusedWindow' | head -2"),
        executeShell("dumpsys window | grep 'mFocusedApp' | head -1"),
      ]);

      const screen = parseScreenState(power.stdout, display.stdout);
      const keyguard = parseKeyguardState(windowPolicy.stdout, focusedWindow.stdout, screen.screenOn);

      return JSON.stringify({
        ok: true,
        battery: battery.stdout.trim(),
        wifi: wifi.stdout.trim(),
        power: power.stdout.trim(),
        display: display.stdout.trim(),
        screenOn: screen.screenOn,
        screenState: screen.screenState,
        keyguardShowing: keyguard.keyguardShowing,
        lockState: keyguard.lockState,
        windowPolicy: windowPolicy.stdout.trim(),
        focusedWindow: focusedWindow.stdout.trim(),
        foreground: activity.stdout.trim(),
        summary:
          keyguard.lockState === "locked"
            ? "Screen is off, so treat the device as effectively locked."
            : keyguard.lockState === "lockscreen_visible"
              ? "Screen is on and the lock screen appears visible."
              : keyguard.lockState === "unlocked"
                ? "Screen is on and the lock screen does not appear visible."
                : "Could not determine lock state conclusively from system dumps.",
      });
    }
  ),
