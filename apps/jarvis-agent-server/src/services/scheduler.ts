...
const JARVIS_AGENT_DIR = join(homedir(), ".jarvis-agent");
const TASKS_FILE = join(JARVIS_AGENT_DIR, "scheduled-tasks.json");
const STATE_FILE = join(JARVIS_AGENT_DIR, "scheduler-state.json");
...
class Scheduler {
  ...
  private persist(): void {
    if (!existsSync(JARVIS_AGENT_DIR)) mkdirSync(JARVIS_AGENT_DIR, { recursive: true });
