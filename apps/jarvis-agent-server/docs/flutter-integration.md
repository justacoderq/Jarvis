# InboxOps API Integration Guide for Flutter

## Overview

InboxOps is an AI agent running on a rooted Android phone (Moto G 5G 2023, Android 13) via Termux + Node.js. It can control the phone's UI, react to notifications, and run tasks on a cron schedule. The backend exposes a REST API over HTTP that the Flutter app will consume.
...
The Flutter app should **automatically discover** the InboxOps server on the local network — no manual IP entry required. On launch (and when reconnecting), the app should:
...
3. Look for a response containing `"agent": "inboxops"` — this confirms it's the right server and not some other service on port 3000.
...
If no server is found, show a "Searching for InboxOps..." state with a retry button. Optionally allow manual IP entry as a fallback in a settings screen, but it should never be the primary flow.
...
- **Connection refused / timeout:** Server is not running or IP changed. Automatically trigger a network rescan. Show "Searching for InboxOps..." while scanning.
