...
# InboxOps - One-time Termux setup script
# Run this on the Android device after installing Termux

set -e

echo "=== InboxOps Termux Setup ==="
...
# Clone and build
echo "[5/5] Clone and build InboxOps..."
if [ ! -d "$HOME/jarvis-agent" ]; then
  cd "$HOME"
  git clone https://github.com/JoshTheMenace/Siri2.git jarvis-agent
else
  echo "  jarvis-agent directory already exists, skipping clone"
fi

cd "$HOME/jarvis-agent"
npm install
npm run build
